...
const AdminPlayersPage: React.FC = () => {
	...
	const handleAssignToTeam = async (playerId: number, teamId: number) => {
      ...
      if (!player?.is_verified) {
        toast.error('Player must be verified before being assigned to a team')
        return
      }
      ...
      // Optimistically update player team
      setPlayers(prev =>
        prev.map(p => p.id === playerId ? { ...p, team: teams.find(t => t.id === teamId) || null } : p)
      );
			...
			toast.error(
				error.response?.data?.message || error.message || "Failed to assign player to team",
			);
		}
	};

	const confirmRemoveFromTeam = async (playerId: number) => {
		setConfirmMessage(
	...
	const confirmDeletePlayer = async (playerId: number) => {
		setConfirmMessage(
	...
	const filteredPlayers = players.filter((player) => {
		...
		const matchesTeam =
			filterTeam === "all" ||
			(filterTeam === "unassigned" && !player.team) ||
			player.team?.name === filterTeam;
	...
	const groupedPlayers = teams.reduce((acc, team) => {
		acc[team.name] = filteredPlayers.filter(
			(player) => player.team?.name === team.name,
		);
		return acc;
				...
				<div className='max-w-7xl mx-auto'>
					...
					<motion.div
						...
						<p className='text-gray-600 dark:text-gray-400'>
							Manage all registered players and their team assignments
						</p>
					</motion.div>
								...
								<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
									{unassignedPlayers.map((player) => (
										<PlayerCard
											...
											onRemoveFromTeam={confirmRemoveFromTeam}
