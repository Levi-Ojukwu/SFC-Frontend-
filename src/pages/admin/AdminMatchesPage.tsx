...
const AdminMatchesPage: React.FC = () => {
	...
	const handleStartMatch = async (match: Match) => {
      ...
      await matchesAPI.startMatch(match.id)
			...
			// Update local state so the UI reflects the live match
			setMatches((prev) =>
				prev.map((m) => (m.id === match.id ? { ...m, is_live: true } : m)),
			);

			toast.success("Match started and is now live!");
		} catch (error: any) {
			toast.error(error.response?.data?.message || "Failed to start the match");
	...
	const handleUpdateScore = async (e: React.FormEvent) => {
		...
		try {
			...
			toast.success(
				"Match score updated successfully! League table will be automatically updated.",
			);
	...
	const openScoreModal = (match: Match) => {
		if (!match.is_live) {
			toast.error("You cannot update the score before the match has started!");
			return; // exit early
		}
						...
						<div className='flex space-x-1 bg-gray-100 dark:bg-dark-800 p-1 rounded-lg w-fit'>
							{[
								{ key: "all", label: "All Matches", count: matches.length },
								{
									key: "upcoming",
									label: "Upcoming",
									count: matches.filter((m) => !m.is_played && !m.is_live)
										.length,
								},
								{
									key: "live",
									label: "Live",
									count: matches.filter((m) => m.is_live).length,
								},
								{
									key: "completed",
									label: "Completed",
									count: matches.filter((m) => m.is_played).length,
								},
							].map((tab) => (
								<button
									key={tab.key}
							...
							<div>
								...
								<select
									...
									onChange={(e) =>
										setMatchForm((prev) => ({
											...prev,
											home_team_id: Number(e.target.value) || null,
										}))
									}
							...
							<div>
								...
								<select
									...
									onChange={(e) =>
										setMatchForm((prev) => ({
											...prev,
											away_team_id: Number(e.target.value) || null,
										}))
									}
									className='w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100'
									required>
			...
			{showScoreModal && selectedMatch && (
				<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto'>
					<div className='bg-white dark:bg-dark-800 rounded-lg p-6 w-full max-w-2xl mx-4 my-8'>
						<h3 className='text-lg font-bold text-gray-900 dark:text-gray-100 mb-4'>
							Update Match Score: {selectedMatch.home_team?.name} vs{" "}
						...
						<form
							...
							<div className='grid grid-cols-2 gap-4'>
								<div>
									<label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>
										{selectedMatch.home_team?.name} Score
									</label>
									<input
										type='number'
										min='0'
										...
										onChange={(e) =>
											setScoreForm((prev) => ({
												...prev,
												home_team_score: Number(e.target.value),
											}))
										}
										className='w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100'
										required
								...
								<div>
									<label className='block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2'>
										{selectedMatch.away_team?.name} Score
									</label>
									<input
										type='number'
										min='0'
										...
										onChange={(e) =>
											setScoreForm((prev) => ({
												...prev,
												away_team_score: Number(e.target.value),
											}))
										}
										className='w-full p-3 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100'
										required
							...
							<div className='bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4'>
								<div className='flex items-start space-x-3'>
									...
									<div className='text-sm text-yellow-700 dark:text-yellow-300'>
										...
										<p>
											When you update this match score, the system will
											automatically:
										</p>
										<ul className='list-disc list-inside mt-2 space-y-1'>
											<li>Update the league table standings</li>
											<li>
												Calculate points, goal difference, and match statistics
											</li>
