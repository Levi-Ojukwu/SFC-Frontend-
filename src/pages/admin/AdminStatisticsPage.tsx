...
const AdminStatisticsPage: React.FC = () => {
	...
	const [activeTab, setActiveTab] = useState<
		"goals" | "assists" | "yellow_cards" | "red_cards" | "handballs"
	>("goals");
	...
	const handleCreateStatistic = async () => {
		try {
			...
			toast.error(
				error.response?.data?.message || "Failed to create statistic",
			);
		}
	};
	...
	const handleViewPlayerDetails = async (playerId: number) => {
		try {
			...
			setSelectedPlayer(response.data.data); // store player detail in state
			setIsModalOpen(true);
		} catch (error: any) {
	...
	return (
		<Layout>
			<div className='min-h-screen p-6'>
				<div className='max-w-7xl mx-auto'>
						...
						<div className='flex items-center justify-between'>
							<div>
								...
								<p className='text-gray-600 dark:text-gray-400'>
									View aggregated statistics and create new match-specific
									records
								</p>
							</div>
							<button
						...
						<div className='flex items-start space-x-3'>
							...
							<div>
								...
								<p className='text-sm text-blue-700 dark:text-blue-300 mt-1'>
									These are aggregated statistics calculated from individual
									match records. To modify statistics, create new match-specific
									records using the "Add Match Statistic" button above.
								</p>
							</div>
						</div>
					</motion.div>
					...
					<motion.div
						...
						<div
							className={`p-6 ${activeTabConfig.bgColor} border-b border-gray-200 dark:border-gray-700`}>
							<div className='flex items-center space-x-3'>
								<activeTabConfig.icon
									className={`w-8 h-8 ${activeTabConfig.color}`}
								/>
								<div>
						...
						<div className='overflow-x-auto'>
							{sortedStatistics.length > 0 ? (
										...
										{sortedStatistics.map((stat, index) => (
											<tr
												...
												<td className='px-6 py-4 whitespace-nowrap text-center'>
													<button
														onClick={() =>
															handleViewPlayerDetails(stat.player.id)
														}
								...
								<div className='text-center py-12'>
									...
									<p className='text-gray-600 dark:text-gray-400'>
										No player statistics match your search criteria.
									</p>
								</div>
							)}
						</div>
										...
										<div>
											...
											<input
												...
												onChange={(e) =>
													setCreateForm((prev) => ({
														...prev,
														yellow_cards: Number(e.target.value),
													}))
												}
												className='w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100'
											/>
										...
										<div>
											...
											<input
												...
												onChange={(e) =>
													setCreateForm((prev) => ({
														...prev,
														red_cards: Number(e.target.value),
													}))
												}
												className='w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100'
											/>
									...
									<div>
										...
										<input
											...
											onChange={(e) =>
												setCreateForm((prev) => ({
													...prev,
													handballs: Number(e.target.value),
												}))
											}
											className='w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-dark-700 text-gray-900 dark:text-gray-100'
										/>
					...
					<motion.div
						...
						<div className='card p-6 text-center'>
							...
							<p className='text-gray-600 dark:text-gray-400'>
								{sortedStatistics.reduce(
									(total, stat) => total + stat[activeTab],
									0,
								)}
							</p>
						</div>
			...
			{isModalOpen && selectedPlayer && (
				<div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
					<div className='bg-white dark:bg-dark-800 rounded-lg p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto'>
						<h3 className='text-lg font-bold text-gray-900 dark:text-gray-100 mb-4'>
							{selectedPlayer.first_name} {selectedPlayer.last_name} (@
							{selectedPlayer.username})
						</h3>
						<p className='text-sm text-gray-600 dark:text-gray-400 mb-4'>
							Team: {selectedPlayer.team || "Unassigned"}
						</p>
						...
						<div className='grid grid-cols-2 gap-4 mb-6'>
							<div>
								...
								{selectedPlayer.statistics.goals}
							</div>
							<div>
								<span className='font-bold'>Assists:</span>{" "}
								{selectedPlayer.statistics.assists}
							</div>
							<div>
								<span className='font-bold'>Yellow Cards:</span>{" "}
								{selectedPlayer.statistics.yellow_cards}
							</div>
							<div>
								<span className='font-bold'>Red Cards:</span>{" "}
								{selectedPlayer.statistics.red_cards}
							</div>
							<div>
								<span className='font-bold'>Handballs:</span>{" "}
								{selectedPlayer.statistics.handballs}
							</div>
						</div>

						{/* Matches */}
						<h4 className='text-md font-semibold mb-2'>Matches</h4>
						{selectedPlayer.matches?.length > 0 ? (
							<ul className='space-y-2 text-sm'>
								{selectedPlayer.matches.map((match: any) => (
									<li
										key={match.id}
